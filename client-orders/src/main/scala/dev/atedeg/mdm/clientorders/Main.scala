...
object Main extends IOApp:
  private val swaggerEndpoint = SwaggerInterpreter().fromEndpoints[IO](
    newOrderEndpoint :: palletizeProductForOrderEndpoint :: orderCompletedEndpoint :: getTransportDocumentEndpoint :: Nil,
    "Client orders",
    Properties.envOrElse("VERSION", "v1-beta"),
  )

  private val swaggerRoute = Http4sServerInterpreter[IO]().toRoutes(swaggerEndpoint)
  private val routes: HttpRoutes[IO] = newOrderRoute
    <+> palletizeProductForOrderRoute
    <+> orderCompletedRoute
    <+> getTransportDocumentRoute
    <+> swaggerRoute
